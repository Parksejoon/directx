...
bool FontShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Studying_DX/font.vs", L"../Studying_DX/font.ps");
	if(!result)
	{
		return false;
	}
											 ...
											 XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 pixelColor)
{
	...
	// 픽셀 상수 버퍼안에 데이터를 가르킬 포인터를 가져옵니다.
	dataPtr2 = (PixelBufferType*)mappedResource.pData;

	// 픽셀 상수 버퍼에 픽셀 컬러를 복사합니다.
	dataPtr2->pixelColor = pixelColor;

	// 픽셀 상수 버퍼의 잠금을 해제합니다.
	deviceContext->Unmap(m_pixelBuffer, 0);

	// 픽셀 셰이더 안에 픽셀 상수 버퍼의 위치를 설정합니다.
	bufferNumber = 0;

	// 갱신된 값이 들어있는 픽셀 셰이더 안에 픽셀 상수 버퍼를 세팅합니다.
