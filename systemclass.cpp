...
bool SystemClass::Initialize()
{
	...
	// 함수에 높이와 너비를 전달하기 전에 변수를 0으로 초기화합니다.
	screenWidth = 0;
	screenHeight = 0;
	...
	// Input 객체 생성. 유저로부터 들어오는 키보드 입력을 받기 위해 생성합니다.
...
void SystemClass::Run()
{
	...
	// 메세지 구조체(MSG)를 초기화합니다.
	ZeroMemory(&msg, sizeof(MSG));

	// 유저로 부터 종료 메세지를 받을 때 까지 루프를 진행합니다.
	done = false;
	while (!done)
	{
		// 윈도우 메세지를 처리
		...
		// 윈도우에서 어플리케이션의 종료를 요청하는 경우 루프를 탈출합니다.
...
void SystemClass::InitiallzeWindows(int& screenWidth, int& screenHeight)
{
	...
	// 풀 스크린 모드 변수의 값에 따라 화면 설정합니다.
	if (FULL_SCREEN)
	{
		// 만약 풀스크린 모드라면 화면 크기를 모니터 크기에 맞추고 색상을 32bit으로 설정합니다.
...
void SystemClass::ShutdownWindows()
{
	...
	// 풀스크린 모드를 빠져나올 때 디스플레이 설정을 변경합니다.
	if (FULL_SCREEN)
	{
